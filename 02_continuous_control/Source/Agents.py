...
class A3C_Agent(BaseAgent):
    def __init__(self, state_size, action_size, network, replay_buffer,
                 n_step_bootstrap = 4, lambda_param = 0.5, seed=0, device="cpu"):
        super().__init__(state_size, action_size, seed, device)
        ...
        self.bootstrap = n_step_bootstrap
    ...
    def act(self, state):
        ...
        self.actor_network.local.eval()
        with torch.no_grad():
            action, value = self.network.local(state).cpu().data.numpy()
        self.network.local.train()
    ...
    def learn(self):
        ...
        # Update Critic Network
        next_actions, values = self.network.target(states)
        ...
        # Losses
        value_loss  = torch.sum( (rewards-values).pow(2) )
        policy_loss = -torch.log(next_actions)*actions - self.beta 
